...
module.exports ={
  ...
  sendReminder                    :   function(req,callback){
    var sems=[],deps=[];
    ...
    var searchCondition = { semester: { $in: sems } , department: { $in: deps }};
    var mailParameters = {
      subject: "Hey you've got some news from the Placement Officer!",
      templateData: {reminder: req.body.reminder},
    ...
    studentController.getEmailIDs(searchCondition,selectArray,populate,function(mails){
      mailParameters.mails=mails;
      emailController.sendMail(mailParameters,'sendReminder',function(error,info){
          callback(error,info);
      });
    });
  }
